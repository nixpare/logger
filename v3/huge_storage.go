...
func (hls *hugeLogStorage) logs() int {
	return hls.n
}
...
func (hls *hugeLogStorage) align(empty bool) {
	if hls.n == 0 {
		return
	}
...
func (hls *hugeLogStorage) needsFlushing() bool {
	return len(hls.buffer) != 0 && memUsageExceeded()
}

func (hls *hugeLogStorage) enableEavyLoad() {
	hls.heavyLoad = true
}

func (hls *hugeLogStorage) disableEavyLoad() {
	hls.heavyLoad = false
}